...
Go-basierter Bot für Diablo II: Resurrected (Offline/Singleplayer). **v0.20.0** unterstützt autonome Farming-Runs für Countess, Mephisto, Summoner, Nihlathak und das Cow Level einschließlich Charakter-Loadouts, Hammerdin-Mephisto, Runtime Replay, Combat, Loot, Sockel-Pickit, Town-Diensten, Mercenary-Support und Desktop-App.
...
```powershell
# Windows-Installer bauen (dist/release/D2R-Offline-Farming-Bot-0.20.0-Setup.exe)
powershell -ExecutionPolicy Bypass -File scripts/build-release.ps1 -Version 0.20.0
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/build-release.ps1 -Version 0.20.0
# Ergebnis: dist\release\D2R-Offline-Farming-Bot-0.20.0-Setup.exe
```

Optional Version taggen (nur für Git-Historie):

```powershell
git tag -a v0.20.0 -m "Release v0.20.0: Reliability, Runtime Replay and Hammerdin"

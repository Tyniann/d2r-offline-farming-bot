...
Lower Kurast bindet für `necro_bone_spear` dieselbe Single-Target-Folge aus einmal Amplify Damage und anschließend Bone Spear ausschließlich an die lokale Objektblocker-Recovery. Die Strategy meldet `RequiresRouteClear() == false`; auf der Reise entsteht deshalb kein Route-Combat. Die Chest-State-Machine liefert nur einen Memory-bestätigten lebenden Gegner im 12-Kachel-Kreis und begrenzt Clear, Retry und Timeout selbst.
...
Die Spaltenzuordnung aus `combat_profiles.*.resources.*.belt_slots` ist der Profil-Default. OperatorSettings Schema 3 darf unter `characters.*.profile_bindings.<profil>.belt_layout` eine vollständige Spaltenkarte (`healing`/`mana`/`rejuvenation` für Slot 1–4) speichern; fehlt sie, bleiben die YAML-Defaults. Gespeicherte Layouts überschreiben zur Runtime Pickup, Trinken und Town-Restock für genau diesen Charakter und bleiben fail-closed bei Teilbelegung oder fehlender Heilspalte bei aktivem Merc. Unter `potion_restock.healing` und `potion_restock.mana` darf derselbe Bindings-Eintrag optionale Akara-Auslöseschwellen speichern; fehlen sie, gelten `town.thresholds`. Gesetzte Werte müssen zwischen 1 und Spalten×4 liegen und ersetzen genau diese Town-Schwellen für den eingefrorenen Loadout. Cow-Readiness hebt sie weiterhin auf das volle Spaltenziel.

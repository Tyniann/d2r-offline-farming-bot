...
Ein Blocker führt vor `Route.Tick` zu `Route.Hold`. Hold bewahrt Segment, Transition und Recovery, übernimmt zunächst bereits innerhalb der Toleranz bestätigte Punkte und verlängert die aktive Deadline nur um deduplizierte echte Hold-Zeit. Hat eine ausdrücklich autorisierte Combat- oder Loot-Bewegung den Spieler auf eine spätere Routenpassage versetzt, stimmt Hold das Playback monoton auf die erste passende spätere Kante innerhalb des unveränderten Driftkorridors ab, aber nur wenn die übersprungene Pfadlänge höchstens der External-Move plus `max_drift_tiles` entspricht. Combat am Cow-Portal darf deshalb nicht auf die Rückschleife der Aufnahme springen. Normales Playback bleibt strikt sequenziell; die Vorwärtsabstimmung darf die Driftgrenze nicht erweitern. `RouteClearExecutor` besitzt ausschließlich Combat-Aktionen. Für `necro_bone_spear` ist genau die Strategie `single_target` registriert: Auf den ersten bestätigten Immediate-/Landing-/Corridor-Blocker einer Blockade wird einmal Amplify Damage gewirkt, danach folgt Bone Spear. `density_relief` überspringt den Curse-Opener. `paladin_hammerdin` nutzt dieselbe `single_target`-Bindung ohne Fluch: Vor dem ersten Hold teleportiert die Pipeline bei mehr als 3 Tiles auf 1 Tile. Ein Aim-only-Tick bleibt retargetbar; sobald Memory irgendein lebendes Monster unter dem Cursor bestätigt und der LMB-Hold tatsächlich startet, bleibt dessen UnitID bis zum Tod gepinnt. Alle drei Snapshots führt erst eine Distanz über 5 Tiles zum Lösen und erneuten Annähern. Lebt dieselbe UnitID nach zwei Sekunden Hold noch, teleportiert die Pipeline zum nächsten anderen lebenden Allowlist-Monster innerhalb von 18 Tiles zum gepinnten Ziel, behält aber die alte UnitID als Hover- und Angriffsziel. Nach bestätigter Bewegung wird der Angriff von der neuen Position versucht, bevor die normale 3-Tile-Annäherung wieder greifen darf. Bereits verwendete Ausweichmonster bleiben ausgeschlossen. Fehlt ein weiteres nahes Ziel, bleibt LMB bis zum gemeinsamen Watchdog unten. Das ist derselbe Boss-Standardangriff, kein paralleler Combat-Pfad.
...
Ein neuerer Memory-Snapshot mit eindeutig positiver Projektion der Spielerbewegung auf den tatsächlich gesendeten Vektor bestätigt einen Teleport sofort. Combat-Teleports klemmen denselben spielbaren Client-Y-Bereich wie Routen-Teleports (höchstens 74 % der Fensterhöhe), damit ein tiefer Zielpunkt den Gürtel nicht aufklappt; Hammer-Aim bleibt davon unabhängig. Das kleine Float-Epsilon toleriert das ganzzahlige D2R-Koordinatengitter; reine Seitwärts-, Stillstands- oder Rückwärtsbewegung zählt nicht. Nur ohne bestätigte Bewegung wartet die Pipeline 500 ms, bevor sie die Landung als blockiert behandelt. Bestätigter Fortschritt setzt Range-Gate und Clear-Watchdog zurück. Ohne Richtungsfortschritt folgen höchstens drei lokale Annäherungsversuche. Deren Erschöpfung sendet keinen weiteren Movement-Input und beendet keinen Run: `route_threat_out_of_range` bleibt ein internes Recovery-Signal. Nur der gemeinsame zusammenhängende No-Progress-Watchdog darf anschließend `route_clear_no_progress` beziehungsweise `cow_combat_no_progress` terminalisieren. Necro-Summoner nähert sich einem nicht projizierbaren Blocker per Force-Move zum nächsten Routenpunkt; Hammerdin-Summoner teleportiert zum Monster. Der zusätzliche Hammerdin-Trefferfallback ist davon getrennt und teleportiert nacheinander zu anderen lebenden Pack-Mitgliedern, während die ursprüngliche UnitID gepinnt bleibt. Fehlt ein Pack-Mitglied innerhalb von 18 Tiles, nutzt er stattdessen einen auf 8 Tiles begrenzten Teleport in Richtung des nächsten validierten Routenpunkts. Beide Annäherungen laufen innerhalb desselben Controller-Holds und dürfen den Mana-Hold nicht umgehen.

Drei frische, lokal vollständige und threat-freie Snapshots schließen eine Blockade ab. Zwölf Sekunden ohne objektiven Ziel-, Count- oder Coverage-Fortschritt enden mit `route_clear_no_progress`. Solange die gepinnte Hammerdin-UnitID lebt, zählen Tode anderer Monster und Coverage-Änderungen nicht als Fortschritt für diesen Sticky-Target-Watchdog. Angriffe, Hold-Start, Hold-Fortsetzung, Zielwechsel und Trankinput setzen den Watchdog nicht zurück.

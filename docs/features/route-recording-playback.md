...
Ein Combat- oder Loot-Hold sendet weiterhin keinen Navigator- oder Transition-Input. Er übernimmt zunächst alle Routenpunkte, die der aktuelle Memory-Snapshot bereits innerhalb der konfigurierten Toleranz bestätigt. Hat eine vom Hold autorisierte externe Bewegung den Spieler stattdessen über Zwischenpunkte hinweg auf einen späteren Routenkorridor versetzt, bindet der Hold das Playback monoton an die erste passende spätere Kante innerhalb des unveränderten `max_drift_tiles`-Korridors, sofern die übersprungene Pfadlänge höchstens der External-Move plus Driftgrenze entspricht. Eine spätere Selbstkreuzung derselben Koordinaten, etwa die Cow-Rückschleife am Portal, zählt nicht als Fortschritt. Gewöhnliches Playback bleibt strikt sequenziell; nur der ausdrücklich gehaltene externe Combat-/Loot-Pfad darf diese Vorwärtsabstimmung auslösen. Nach einem plausiblen Vorwärtssprung kann deshalb keine Recovery zu einem durch den eigenen Combat- oder Loot-Teleport überholten Punkt zurückspringen.

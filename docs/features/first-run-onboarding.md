...
Im Charakterschritt ist `session.character` nur eine optionale Startvorauswahl. Ein frischer Root darf deshalb einen lokalen Charakter auswählen, sobald dessen Save als regulärer Dateiname sichtbar ist, der aktuelle Run einen Combat-Profil-/Klassenkontext besitzt und die globalen sowie charakterbezogenen Bildschirmbestätigungen valide sind. Nicht vorbereitete Saves bleiben sichtbar, erben aber nicht pauschal die Klasse des aktuellen Runs. Statt interner Begriffe zeigt die Oberfläche getrennt an, ob ein unterstütztes Kampfprofil fehlt, welche Klassen derzeit unterstützt werden oder ob die automatische D2R-Auswahl noch nicht eingerichtet ist. Die Stepper- und Inhaltslisten besitzen eigene einspaltige Layoutregeln und übernehmen nicht die Dreispaltenstruktur fachlicher Listen.

Recording-Voraussetzungen werden unmittelbar nach einer erfolgreichen Charakterbestätigung neu vom Core geladen; ein vor der Auswahl geladener leerer Pickit-Kontext darf nicht als veralteter Sperrgrund stehen bleiben. Stabile technische Gründe werden deutsch erklärt. Für künftig geführte Charaktereinrichtung ist zusätzlich vorgesehen, die mitgelieferte runbezogene Standard-Pickit-Kette automatisch pro Charakter und unterstütztem Run zuzuordnen.

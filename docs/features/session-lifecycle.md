...
| `run_restartable` | live validierter `hard_stuck`, `route_drift_exceeded`, `route_segment_timeout`, `route_transition_failed`, `stash_approach_failed`, `cow_portal_hover_not_found` oder `hammerdin_cta_skill_unconfirmed` | Run abbrechen, Fehlerbudget abbuchen; Game-Restart nur über einen validierten sicheren Exit-Flow. |

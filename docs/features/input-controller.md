...
- **Wichtige Dateien:**
  ...
  - `chat.go` — `SendChatCommand` für den allowlisted `/players 1`–`/players 8`-Chat; Befehlstext per virtueller Taste (`VkKeyScanW`), Enter als virtueller Key
...
- **Fokus an semantischen Aufrufern:** Phase-7-Menüaktionen und positionsgebundene Profilcasts aktivieren und bestätigen D2R explizit; Low-Level-Run-/Test-Primitives besitzen nicht automatisch denselben semantischen Fokus-Guard.

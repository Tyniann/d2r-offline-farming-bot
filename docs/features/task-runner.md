...
Ist derselbe Blocker drei frische Snapshots lang nicht projizierbar, nähert die Pipeline kontrolliert an. Necro-Summoner verwendet Force Move zum bereits validierten nächsten Routenpunkt. Hammerdin-Summoner teleportiert zum Monster auf 1 Tile, sobald die Distanz vor dem ersten Hold über 3 Tiles liegt. Nach gestartetem Hold bleibt die UnitID bis zum Tod gepinnt; alle drei Snapshots löst erst eine Distanz über 5 Tiles das erneute Annähern aus. Ein Mana-Hold unterbindet neue Teleports. Nach 500 ms und einem frischen Snapshot muss die Distanz zur eingefrorenen Zielposition mindestens ein Tile kleiner sein. Fortschritt fordert einen neuen Projektionsbeleg; ohne Fortschritt sind höchstens drei Versuche erlaubt. `Route.Tick` bleibt dabei gesperrt; ausgeschöpfte Hammerdin-Teleports dürfen den gemeinsamen `route_clear_no_progress`-Watchdog nicht umgehen.
...
Ein produktives `town_portal_enter_failed` umgeht `retry-return`. Nach dem einmaligen Teleport-Workaround im Enter-Step folgt direkt der Memory-gated Save-&-Exit und derselbe Queue-Index. `cow_return_portal_failed` nutzt denselben Direkt-Exit, nachdem der zweite hover-bestätigte Tristram-Klick auf derselben Portal-UnitID ebenfalls ohne Ortswechsel bleibt.

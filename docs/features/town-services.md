...
`mercenary_heal` und `mercenary_revive` sind gegenseitig exklusiv aus einem Snapshot; Dead hat Vorrang. Heal: Akara hover-bestätigt anklicken, Full-HP bestätigen; Dialog wird nicht geschlossen und verschwindet beim nächsten Walk. Revive: Kashya hover-bestätigt anklicken, Home/Down/Enter je Tick genau einmal; weiterhin eindeutig Dead nach Enter → `mercenary_revive_insufficient_gold` (Queue stoppt nicht-retrybar). Nimmt D2R einen ersten NPC-Klick wegen eines bewegten NPCs nicht an, wird dieselbe gepinnte Runtime-UnitID nach 750 ms genau einmal an ihrer aktuellen Position neu erfasst; mehr als zwei bestätigte Klicks oder ein Blindklick sind ausgeschlossen. NotHired und Invalid bleiben im Run-Readiness-Gate terminal. Seit Phase 20.7 autorisiert Dead-at-start dagegen genau den bestehenden Town-Plan samt Kashya-Revive, bevor der produktive Task starten darf. Gate 18.3 live bestanden. Gate 18.4: vier `waypoint-kashya`-Varianten aktiviert und je Layout Playback WP-Roundtrip + Stash→Kashya→WP grün.

### Cow-Startreserve (Phase 20.7)

Für `cows` verwendet die Readiness denselben Akara-Restock wie die übrigen Routen. Gürtelbelegung und Nachkaufschwellen kommen aus den Charakter-Einstellungen (`belt_layout` und `potion_restock`); `town.thresholds` bleibt der Fallback. Planung, Goldgate, Akara-Navigation, atomarer Kauf und Memory-Verifikation bleiben exakt die vorhandenen Restock-Verträge. Gezählt werden nur passende Tranktypen in ihren konfigurierten Spalten. Leere kaufbare Spalten werden auf das Spaltenziel aufgefüllt; ein Seed-Trank ist nicht erforderlich. Regenerationstränke bleiben nicht bei Akara kaufbar. Eine automatische Entnahme aus dem 99er-Stash existiert nicht.

Die Live-Evidenz vom 8. August 2026 zeigte visuell drei Healing- und sechs Mana-Potions, während der profilgebundene World-Read `Healing=1` und `Mana=2` meldete. Ursache war keine reduzierte Item-Projektion: D2R kodiert die 16 Belt-Positionen flach in `GridX` 0–15, während der alte Zähler nur `GridX` 0–3 als Spalten behandelte. Der Zähler dekodiert deshalb die Spalte mit `GridX mod 4`, zählt jede Zeile und verwendet danach wieder den bestehenden Restock-Vertrag vollständig. Es gibt keinen Kauf-Retry und keine automatische Stash-Entnahme.

...
Für `cows` werden die Triggerwerte der kaufbaren Healing-/Mana-Ressourcen im Readiness-Plan einmalig auf die vollständigen Ziele der jeweils zugewiesenen Belt-Spalten angehoben. Planung, Goldgate, Akara-Navigation, atomarer Kauf und Memory-Verifikation bleiben exakt die vorhandenen Restock-Verträge. Gezählt werden nur passende Tranktypen in ihren konfigurierten Spalten. Mindestens eine Rejuvenation Potion muss in einer Rejuvenation-Spalte belegt sein; fehlt sie, endet Readiness ohne Händlerinput mit `cow_rejuvenation_reserve_missing`, weil Akara diesen Bestand nicht ersetzen kann. Eine automatische Entnahme aus dem 99er-Stash existiert nicht.

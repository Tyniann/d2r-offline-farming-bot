...
Die Definition pinnt den Summoner über NPC-ID `250` (`world.Summoner`) ohne Super-Unique-Gate. `BossEngageSequence` ist leer — kein Bone Prison; nach Acquire folgt direkt der profilgebundene Standardangriff. Für `necro_bone_spear` ist das Bone Spear, für `paladin_hammerdin` der Blessed-Hammer-LMB-Hold aus dem Mephisto-Pfad. Kill-Confirm wie bei Mephisto über Abwesenheit der gepinnten UnitID. Als bewusst enger Summoner-Sonderfall gilt der Boss nach vollständig abgespielter Route auch dann als erledigt, wenn die Priority-Enumeration ihn beim unmittelbar folgenden Acquire nicht mehr enthält: Der schwache Boss kann bereits durch den Mercenary oder einen durchschlagenden Route-Clear-Angriff sterben. Die Pipeline überspringt dann den Bossangriff und setzt an der aktuellen terminalen Spielerposition mit Restgegnern und Loot fort; andere Runs behalten ihre strikte Boss-Akquise.

Im Full Run folgt danach und vor dem Teleport zur Bossleiche der opt-in Step `clear_nearby_hostiles` für `necro_bone_spear`: Er ermittelt in jedem Snapshot neu den nächsten lebenden Specter-, Hell-Clan- oder Ghoul-Lord-Typ innerhalb von 18 Tiles, zielt auf dessen aktuelle Memory-Position und castet den normalen `attack_skill` erst nach Hover-Bestätigung genau dieser Monster-`UnitID`. Drei gegnerfreie Snapshots oder höchstens 20 tatsächlich gesendete Angriffe beenden die best-effort Bereinigung; anschließend folgt die Loot-Repositionierung auch bei verbleibenden Gegnern. Hammerdin überspringt den Step, weil Blessed Hammer als Flächenangriff die meisten Restgegner bereits wegräumt. Die isolierte `boss`-Phase endet weiterhin direkt nach Kill-Confirm.
...
Seit Phase 17.2 ist `runs.definitions.summoner.route_combat` presence-sensitiv validiert und standardmäßig aktiv; `enabled: false` bleibt der Rollback. Seit Phase 17.3 prüft der produktive Route-Step vor jedem Movement Immediate-, Corridor- und Landing-Zone. Bei einem Blocker hält er den unveränderten RoutePlayer. `necro_bone_spear` wirkt einmal Amplify Damage (`F1` im Default) auf das erste hover-bestätigte Ziel und greift anschließend stationär per `single_target`/Bone Spear an. `paladin_hammerdin` teleportiert vor dem ersten Hold bei mehr als 3 Tiles auf 1 Tile und hält Blessed Hammer, sobald irgendein lebendes Monster unter dem Cursor liegt. Diese UnitID bleibt danach bis zu ihrem Tod gepinnt; alle drei Snapshots führt erst eine Distanz über 5 Tiles zum Lösen und erneuten Annähern. Überlebt sie zwei Sekunden stationären Hold, teleportiert der Bot zum nächsten anderen lebenden Allowlist-Monster innerhalb von 18 Tiles zum gepinnten Ziel, behält aber die alte UnitID für Hover und Hammer-Hold. Fehlt ein solches Monster, teleportiert er höchstens 8 Tiles in Richtung des nächsten validierten Routenpunkts statt zu einem weit entfernten Gegner. Von der neuen Position wird zuerst der Hold auf das alte Ziel versucht; erst ein bestätigter Projektionsfehler darf wieder zum alten Ziel annähern. Bereits verwendete Ausweichziele werden übersprungen. Bestätigte Positionsänderung beendet das Teleport-Settle sofort, 500 ms bleiben nur die Frist für eine blockierte Landung. Konzentrationsauswahl und Cursor-Aim laufen im selben Tick, der Hold weiterhin erst nach Memory-Bestätigung. Ein aktiver Mana-Hold verhindert neue Teleports, ohne das endliche No-Progress-Budget zu umgehen. Drei lokal vollständige freie Snapshots und ein weiterer frischer Tick sind für die Fortsetzung nötig. Candidate-Playback und Recording bleiben davon unberührt; der Summoner-Boss sowie die anderen registrierten Hammerdin-Bosse verwenden denselben begrenzten Trefferfallback.

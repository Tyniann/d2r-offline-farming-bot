...
Der produktive Queue-Start verwendet denselben Selector erneut, wenn Memory kein offenes Rogue Encampment bestätigt – auch nach Apply, solange D2R noch auf dem Charakterbildschirm steht. Er löst den konfigurierten Charakter frisch aus dem lokalen Save-Katalog auf und führt die begrenzte Home-/Down-Suche unabhängig von der zufällig in D2R markierten Zeile aus. Erst der bestätigte Charakteranker erlaubt den bestehenden Play-/Difficulty-Ablauf; nach Eintritt bleiben Character Name, Klasse und Rogue Encampment per Memory autoritativ. Der Queue-Start besitzt keine zweite Navigationslogik und setzt niemals voraus, dass der gewünschte Charakter bereits ausgewählt ist. Nach Save & Exit wartet bereits der Selector vor seinem ersten Screenshot das begrenzte 1,2-Sekunden-Renderfenster ab. Bleiben Play- und Dialog-Anker danach in einem einzelnen Renderframe uneindeutig, prüft er innerhalb des bestehenden Gesamt-Timeouts im normalen 350-ms-Takt erneut; ein klar erkannter Difficulty-Dialog bleibt terminal. Memory-`menu` allein darf weder einen noch im Übergang gezeichneten Charakterbildschirm ablehnen noch Home/Down freigeben. Fehler protokollieren die konkrete Startstufe; der Supervisor projiziert `game_start_failed` mit einer bleibenden deutschen Erklärung.
...
D2R muss bereits auf dem Offline-Charakterbildschirm bei 1280×720 stehen, und das Fenster muss eine messbare Clientfläche haben. Ein Timeout mit „keine nutzbare Fläche“ bedeutet, dass der Core nur ein 0×0-HWND gesehen hat (minimiert, Vollbild-Alt-Tab oder falsches Owner-Fenster) und den Charakterbildschirm deshalb gar nicht prüfen konnte. Das Dashboard-Dropdown wählt nur, wen Apply in D2R anwählt. Queue, Run-Karten und Start gehören zum bestätigten Farm-Charakter; nach erfolgreichem Apply lädt die gespeicherte Queue des neuen Charakters. Deaktivierte Charaktere bleiben im Dropdown als nicht verfügbar sichtbar.

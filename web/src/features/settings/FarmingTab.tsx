...
export function FarmingTab({
  draft, saved, selectedCharacter, characterNames, runs, mutable, restartRequired, diffPaths,
    ...
    <section className={queueChanged ? "settings-field-changed" : undefined}>
      ...
      {characterNames.length === 0 || !characterSettings
        ...
        : <div className="settings-grid two-columns settings-form-width">
          <label>Charakter<select value={selectedCharacter} onChange={(event) => onSelectCharacter(event.target.value)} disabled={!mutable}>{characterNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label>

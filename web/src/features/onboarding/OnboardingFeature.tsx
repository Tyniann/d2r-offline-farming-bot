...
function prerequisiteStatus(entry: { ready: boolean; reason?: string }): string {
  return entry.ready ? "bereit" : prerequisiteReasons[entry.reason ?? ""] ?? entry.reason ?? "fehlt";
}
...
export function OnboardingFeature({ status, catalog, onRefresh, onClose, onOpenRoutes, initialStep = 0 }: Props) {
  ...
  const [character, setCharacter] = useState(status.selection.character || catalog.characters.find((entry) => entry.selectable)?.name || "");
  ...
  return <section className="onboarding" aria-labelledby="onboarding-title">
    ...
    {step === 5 && <div className="onboarding-panel"><h2>Charakter und Schwierigkeit bestätigen</h2><p>Alle gefundenen lokalen Charaktere werden angezeigt. Verfügbar sind nur Charaktere, die der Bot mit einem unterstützten Kampfprofil und einer sicheren automatischen D2R-Auswahl kennt. Derzeit unterstützte Klasse: <strong>{supportedCharacterClasses(catalog)}</strong>.</p><div className="selection-grid"><label>Charakter<select value={character} onChange={(event) => setCharacter(event.target.value)}>{catalog.characters.map((entry) => <option key={entry.slug} value={entry.name} disabled={!entry.selectable}>{entry.name}{entry.selectable ? "" : " – nicht verfügbar"}</option>)}</select></label><label>Schwierigkeit<select value={difficulty} onChange={(event) => setDifficulty(event.target.value)}>{catalog.difficulties.map((entry) => <option key={entry.id} value={entry.id}>{entry.display_name}</option>)}</select></label><Button disabled={busy || !character || !effectiveInputReady || !catalog.characters.some((entry) => entry.name === character && entry.selectable)} onClick={() => void submitSelection()}>Über Core bestätigen</Button></div><p>Aktiv: {status.selection.character ? `${status.selection.character} / ${status.selection.difficulty}` : "noch nicht bestätigt"}</p>{catalog.characters.some((entry) => !entry.selectable) && <><h3>Warum sind Charaktere nicht verfügbar?</h3><ul className="character-availability">{catalog.characters.filter((entry) => !entry.selectable).map((entry) => <li key={entry.slug}><strong>{entry.name}</strong><span>{characterAvailabilityText(entry, catalog)}</span></li>)}</ul></>}</div>}
    {step === 6 && <div className="onboarding-panel"><h2>Core-Readiness</h2><p>Die Liste stammt aus Status-, Katalog- und Recording-API.</p><ul className="readiness-list"><li><span>Versionsgate</span><strong>{compatibility === "compatible" ? "bereit" : "fehlt"}</strong></li><li><span>Charakter/Difficulty</span><strong>{status.selection.character ? "bereit" : "fehlt"}</strong></li><li><span>Input</span><strong>{effectiveInputReady ? "bereit" : "abgelehnt/deaktiviert"}</strong></li>{(selectedOption?.prerequisites ?? []).map((entry) => <li key={entry.id}><span>{prerequisiteLabels[entry.id] ?? entry.id}</span><strong>{prerequisiteStatus(entry)}</strong></li>)}</ul><Button variant="secondary" onClick={() => void load()}>Readiness neu laden</Button></div>}
    {step === 7 && <div className="onboarding-panel"><h2>Erste echte Route</h2><p><strong>So startest du:</strong> Öffne unten den Routenbereich und klicke dort beim gewünschten Run auf „Aufnahme starten“. Bleibe am Startwegpunkt stehen, bis der Core „Aufnahme läuft“ meldet. <strong>{hotkeys?.recording_finish ?? "F9"} startet keine Aufnahme</strong>, sondern beendet ausschließlich eine bereits laufende Aufnahme an der gewünschten Kampfposition.</p><div className="route-choice"><label><input type="radio" name="first-route" checked={routeID === "countess"} onChange={() => setRouteID("countess")} /> Countess <strong>empfohlen</strong></label><label><input type="radio" name="first-route" checked={routeID === "mephisto"} onChange={() => setRouteID("mephisto")} /> Mephisto</label></div>{selectedOption && <><p>{selectedOption.instructions_de}</p><ul>{(selectedOption.prerequisites ?? []).map((entry) => <li key={entry.id}>{prerequisiteLabels[entry.id] ?? entry.id}: {prerequisiteStatus(entry)}</li>)}</ul></>} {["failed_safe", "emergency_cancelled"].includes(workflowState) && <StateMessage kind="error" title="Vorherige Aufnahme wurde verworfen">Es gibt kein Resume. Starte die Aufnahme sauber neu.</StateMessage>}<Button disabled={!allPrerequisitesReady || !settings?.input.enabled || compatibility !== "compatible"} onClick={() => onOpenRoutes(routeID)}>Routenbereich öffnen und Aufnahme starten <ExternalLink aria-hidden="true" size={17} /></Button></div>}

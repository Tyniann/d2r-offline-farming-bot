...
describe("runReasons", () => {
  ...
  it("unterscheidet Klassenkonflikt und fehlende Strategy", () => {
    ...
    expect(runAvailabilityText("unavailable", ["profile_run_strategy_unavailable"], "paladin", i18n.t).detail).toBe("Für dieses Kampfprofil ist dieser Run noch nicht freigegeben.");
  ...
  it("präsentiert strukturierte API-Fehler ohne Satzfragment-Parsing", () => {
    expect(queueStartErrorText(new ApiError("queue_entry_unavailable", { run_id: "mephisto" }, "req-1", 409), i18n.t)).toBe("Ein Run in der Reihenfolge ist für diesen Charakter nicht startfähig.");

...
async function mockRoutes(page: Page): Promise<void> {
  ...
  const option = (runID: string, startWaypoint: string) => ({ run_id: runID, display_name: runID, instruction_code: `record_${runID.replace("-", "_")}`, start_kind: "waypoint", start_waypoint: startWaypoint, allowed_start_area_id: 1, allowed_route_area_ids: [1], terminal_area_id: 1, terminal_max_distance_tiles: 80, available: true, prerequisites: [] });

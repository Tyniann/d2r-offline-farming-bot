...
import { DesktopSettingsStore, desktopSettingsDefaults } from "./desktop-settings.js";
...
describe("DesktopSettingsStore", () => {
  ...
  it.each([
    ...
    ["unbekanntes Schema", { schema_version: 2, autostart: true, onboarding_completed: true }],
  ...
  it("behält bei einem Fehler vor dem Replace die alte Datei bytegleich", async () => {
    ...
    await expect(store.save({ schema_version: 1, autostart: true, onboarding_completed: true })).rejects.toThrow("simulierter Replace-Fehler");

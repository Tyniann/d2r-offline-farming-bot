...
func verboseGroundItemsHint(cur world.State) string {
	...
	for _, item := range items {
		...
		parts = append(parts, fmt.Sprintf("unit=%d id=%d code=%q type=%q name=%q quality=%s identified=%t ethereal=%t flags=0x%X active_stat194=%s base_stat194=%s sockets=%d sockets_available=%t socketed=%t identity_kind=%q identity_raw_id=%d identity_available=%t identity_name=%q identity_consistent=%t identity_reason=%q x=%d y=%d",
			item.UnitID,
			item.TxtFileNo,
			item.Code,
			item.Type,
			...
			item.Ethereal,
			item.Flags,
			world.FormatSocketStatEvidence(item.SocketStatActive),
			world.FormatSocketStatEvidence(item.SocketStatBase),
			item.Sockets,
			item.SocketsAvailable,
			item.Socketed,
			item.IdentityKind,
			item.IdentityRawID,
...
func verboseInventoryItemsHint(items []world.Item) string {
	...
	for _, item := range items {
		...
		parts = append(parts, fmt.Sprintf("unit=%d id=%d code=%q name=%q quality=%s identified=%t ethereal=%t flags=0x%X active_stat194=%s base_stat194=%s sockets=%d sockets_available=%t socketed=%t identity_kind=%q identity_raw_id=%d identity_available=%t identity_name=%q identity_consistent=%t identity_reason=%q grid=%d,%d size=%dx%d",
			item.UnitID,
			item.TxtFileNo,
			item.Code,
			name,

...
func TestResolveWaypointTargetsInspectReportIsStableAndReadOnly(t *testing.T) {
	...
	if report.SchemaVersion != 1 || len(report.Targets) != 7 {
		t.Fatalf("report = %+v", report)
	}
	if report.Targets[0].ID != pathing.WaypointTargetArcaneSanctuary || report.Targets[1].ID != pathing.WaypointTargetBlackMarsh || report.Targets[2].ID != pathing.WaypointTargetDuranceOfHateLevel2 || report.Targets[3].ID != pathing.WaypointTargetHallsOfPain || report.Targets[4].ID != pathing.WaypointTargetLowerKurast || report.Targets[5].ID != pathing.WaypointTargetRogueEncampment || report.Targets[6].ID != pathing.WaypointTargetStonyField {

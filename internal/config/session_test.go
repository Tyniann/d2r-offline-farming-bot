...
func TestSessionPhase28DefaultRetryClassesGainHammerdinCTA(t *testing.T) {
	var cfg SessionConfig
	cfg.RetryClasses = append([]string(nil), phase28SessionRetryClasses...)
	cfg.applyDefaults()
	if !reflect.DeepEqual(cfg.RetryClasses, defaultSessionRetryClasses) {
		t.Fatalf("migrated retry classes = %v, want %v", cfg.RetryClasses, defaultSessionRetryClasses)
	}
	...
	for _, class := range cfg.RetryClasses {
		if class == "hammerdin_cta_skill_unconfirmed" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("default retry classes = %v, want hammerdin_cta_skill_unconfirmed", cfg.RetryClasses)
	}
}

func TestSessionPhase27DefaultRetryClassesGainCowPortalHover(t *testing.T) {
	var cfg SessionConfig
	cfg.RetryClasses = append([]string(nil), phase27SessionRetryClasses...)
	cfg.applyDefaults()
	if !reflect.DeepEqual(cfg.RetryClasses, defaultSessionRetryClasses) {
		t.Fatalf("migrated retry classes = %v, want %v", cfg.RetryClasses, defaultSessionRetryClasses)
	}
	found := false
	for _, class := range cfg.RetryClasses {
		if class == "cow_portal_hover_not_found" {
			found = true
			break
		}
	}
...
func TestSessionPhase30DefaultRetryClassesGainCowLegPickup(t *testing.T) {
	var cfg SessionConfig
	cfg.RetryClasses = append([]string(nil), phase30SessionRetryClasses...)
	cfg.applyDefaults()
	if !reflect.DeepEqual(cfg.RetryClasses, defaultSessionRetryClasses) {
		t.Fatalf("migrated retry classes = %v, want %v", cfg.RetryClasses, defaultSessionRetryClasses)
	}
	found := false
	for _, class := range cfg.RetryClasses {
		if class == "cow_leg_pickup_failed" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("default retry classes = %v, want cow_leg_pickup_failed", cfg.RetryClasses)
	}
}
